...
#### 1.1 领域驱动设计（DDD）与简洁架构

当我们写代码时，如果遇到什么功能就随便找个地方塞进去，项目一变大，代码就会乱成一锅粥。为了解决这个问题，程序员们总结出了两套非常好用的“整理收纳术”：**领域驱动设计（DDD）** 和 **简洁架构（Clean Architecture）**。

特别是在用 C++ 开发像游戏引擎这样复杂的东西时，这两套心法能帮我们把最核心的“物理法则”和最外围的“图形渲染（比如 Vulkan/OpenGL）”彻底拆开，谁也不连累谁。



##### 1.1.1 领域驱动设计（DDD）介绍

简单来说，**DDD 就是教你怎么把代码写得像“人话”。** 过去我们写代码是“围着数据库转”或者“围着计算机内存转”，而 DDD 要求我们“围着现实世界的业务转”。它给我们提供了四个最核心的“编程套路”，在 C++ 里它们其实都是普通的 `class` 或 `struct`，只是我们要给它们立下不同的规矩：

**1. 值对象（Value Object）：只看属性，不分你我**

- **大白话：** 就像一张 100 元钞票。你不在乎这张钞票出厂编号是多少，你只在乎它能买 100 块钱的东西。只要面值一样，它们就是一样的。
- **规矩：** 不能被修改（想改就直接换个新的），不用给它分配什么唯一的 ID。
- **引擎里的例子：** 空间坐标 `Vector3(x, y, z)` 或者颜色 `Color`。两个完全一样的坐标点就是同一个点。

**2. 实体（Entity）：靠“身份证号”活着的专属对象**

- **大白话：** 就像你的身份证。你今天穿红衣服，明天穿绿衣服，甚至改了名字，但只要身份证号不变，你还是你。
- **规矩：** 必须给它分配一个独一无二的 ID。为了防止世界上出现两个一样的人，一般不允许“拷贝（Copy）”它。
- **引擎里的例子：** 场景里的游戏对象 `GameObject`。一个怪物从地图东边走到西边（坐标变了），但它依然是原本那个怪物，系统靠它的 ID 来认人。

**3. 聚合与聚合根（Aggregate & Aggregate Root）：团队与包工头**

- **大白话：** 把几个关系紧密的对象打包成一个“团队”，并选出一个“包工头（聚合根）”。外面的代码如果想让团队干活，只能去找包工头，绝对不能私自去找团队里的小弟。
- **规矩：** 包工头负责管理小弟的生老病死（内存分配与释放），保护数据不被外部乱改。
- **引擎里的例子：** `Scene`（场景）就是包工头，里面装满了 `GameObject`（小弟）。你想在场景里刷个新怪物，不能直接捏造一个塞进内存，必须调用场景提供的方法：`Scene::AddGameObject()`。

**4. 领域服务（Domain Service）：跨部门协调员**

- **大白话：** 有些复杂的业务，牵扯到好几个对象，交给谁做都不合适，这时候就专门成立一个“服务中心”来办这件事。
- **规矩：** 这是一个没有自己状态（没有成员变量）的类，只负责处理复杂的运算逻辑。
- **引擎里的例子：** `CollisionResolver`（碰撞处理）。物体 A 和物体 B 撞在一起，算谁的？都不合适。不如把它俩一起扔给“碰撞协调员”来算出结果。



##### 1.1.2 领域驱动设计与简洁架构的关系

如果说 **DDD 是代码的“灵魂与剧本”**，教你怎么表达业务逻辑；那么 **简洁架构就是代码的“舞台与围墙”**，教你怎么摆放这些代码。

简洁架构的核心规矩只有一条：**像剥洋葱一样分层，外层可以依赖内层，但内层绝对不准看外层一眼。**

当这两人强强联手时，在你的 C++ 引擎项目里就会形成一个非常清爽的“同心圆”结构（从最核心向外看）：

| **架构分层（洋葱圈）**    | **相当于公司的什么角色？**                               | **在引擎里是什么样子？**                                     |
| ------------------------- | -------------------------------------------------------- | ------------------------------------------------------------ |
| **1. 领域层（最内层）**   | **核心大脑（CEO）** 掌握公司所有核心机密，但不接触客户。 | 存放上面提到的 `Vector3`、`GameObject` 等。**全都是纯 C++ 代码，绝对不准出现任何底层图形库或第三方库的影子。** |
| **2. 应用层**             | **部门经理** 听取需求，指挥 CEO 干活，本身不创造规则。   | 编写 `LoadScene()` 或 `UpdatePhysics()` 等用例代码。负责调兵遣将。 |
| **3. 接口适配器**         | **翻译官** 把公司的机密文件，翻译成外面能听懂的话。      | 将领域层里算好的 `Vector3` 坐标，翻译成 OpenGL 或者 Vulkan 能认出来的顶点数据格式。 |
| **4. 基础设施（最外层）** | **快递员/接待员** 负责和外面的真实世界打交道。           | 处理 Windows 操作系统的窗口、获取鼠标键盘的点击、或者是处理底层文件的读写。 |
